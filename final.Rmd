...
chisq.test(train$gender,train$readmitted)
chisq.test(train$age,train$readmitted)
chisq.test(train$max_glu_serum,train$readmitted)
chisq.test(train$A1Cresult,train$readmitted)
chisq.test(train$metformin,train$readmitted)
chisq.test(train$medical_specialty,train$readmitted)
chisq.test(train$repaglinide,train$readmitted)
chisq.test(train$nateglinide,train$readmitted)
chisq.test(train$chlorpropamide,train$readmitted)
chisq.test(train$glimepiride,train$readmitted)
chisq.test(train$glipizide,train$readmitted)
chisq.test(train$glyburide,train$readmitted)
chisq.test(train$pioglitazone,train$readmitted)
chisq.test(train$rosiglitazone,train$readmitted)
chisq.test(train$acarbose,train$readmitted)
chisq.test(train$miglitol,train$readmitted)
chisq.test(train$tolazamide,train$readmitted)
chisq.test(train$insulin,train$readmitted)
chisq.test(train$glyburide.metformin,train$readmitted)
chisq.test(train$change,train$readmitted)
chisq.test(train$diabetesMed,train$readmitted)
...
#library(groupdata2)
#data <- fold(
#  data = train1[, -c(25,28,33,35,36,39,40,41,42)], k = 5,
 # cat_col = 'readmitted') %>% 
 # arrange(.folds)
#library(caTools)
#library(caret)
#ind=createDataPartition(dat2$readmitted,list=FALSE)
#traindat<-dat2[ind,]
#testdat<-dat2[-ind,]
#controlpara<-trainControl(method="cv",number=5,savePredictions = TRUE,classProbs = TRUE)
#praragrid<-expend.grid(readmitted=c(0,1))
#modelrandom<-train(readmitted ~ admission_type_id + Length.of.Stay + num_procedures + num_medications + number_outpatient+ number_emergency + number_inpatient + number_diagnoses + diabetesMed+(1|patient_nbr),data=traindat,method="rf")

#lrm.finalbiclmm <- lrm(readmitted ~ admission_type_id + Length.of.Stay + num_procedures + num_medications + number_outpatient+ number_emergency + number_inpatient + number_diagnoses + diabetesMed+(1|patient_nbr),data = train1[, -c(25,28,33,35,36,39,40,41,42)])
#cross.caliblclmm <- calibrate(lrm.finalbiclmm, method="crossvalidation", B=10) # model calibration
#par(family = 'serif')
#plot(cross.caliblassosig, las=1, xlab = "Predicted Probability")
#plot(lrm.finalbiclmm)
library(pROC)
